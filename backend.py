...
def list_players() -> str:
    ...
    return "All Players:\n\n" + "\n".join([f'id={p.doc_id}, name={p["name"]}, rating={p["mu"]}' for p in players.all()])
...
def validate_players(team: list[int]) -> list[Document]:
    ...
    if not team:
        raise ValueError('A team cannot be empty')
    
    team_players = cast(list[Document],players.get(doc_ids=team))
    if (unadded:=set(team) - set([p.doc_id for p in team_players])):
        raise ValueError(f'Player IDs {list(unadded)} are not in the database, add them first.')
...
def update_ratings(winners: list[int], losers: list[int]) -> str:
    ...
    rating_groups = [{p.doc_id:Rating(p['mu'], p['sigma'])for p in winner_players}, {p.doc_id:Rating(p['mu'], p['sigma']) for p in loser_players}]
...
def get_win_prob(team1: list[int], team2: list[int]) -> str:
    ...
    team1_players = [Rating(p['mu'], p['sigma']) for p in validate_players(team1)]
    team2_players = [Rating(p['mu'], p['sigma']) for p in validate_players(team2)]
    return f'{_get_win_prob(team1_players, team2_players): .1f}'
